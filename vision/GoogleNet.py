...
"""A simple implementation of GoogleNet(Inception Net)
...
class Inception_block(nn.Module):
    def __init__(self, in_channels, out_11, red_33, out_33, red_55, out_55, out_11pool):
        ...
        self.branch2 = nn.Sequential(
            conv_block(in_channels, red_33, kernel_size=1),
            conv_block(red_33, out_33, kernel_size=3, padding=1)
            )
        self.branch3 = nn.Sequential(
            conv_block(in_channels, red_55, kernel_size=1),
            conv_block(red_55, out_55, kernel_size=5, padding=2)
            )
        self.branch4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),
            conv_block(in_channels, out_11pool, kernel_size=1)
            )

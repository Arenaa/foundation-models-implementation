...
Tan, M., & Le, Q. (2019, May). Efficientnet: Rethinking model scaling for
...
class SqueezeEcitation(nn.Module):
...
class InvertedResidualBlock(nn.Module):
    def __init__(self,
        ...
        reduced_dim = int(in_channels / redution)
        ...
        self.conv = nn.Sequential(
            CNNBlock(
                hidden_dim, hidden_dim, kernel_size, stride, padding, group=hidden_dim
            ),
            SqueezeEcitation(hidden_dim, reduced_dim),
            nn.Conv2d(hidden_dim, out_channels, 1, bias=False),
            nn.BatchNorm2d(out_channels)
        )

    def stocastic_depth(self, x):
    ...
    def forward(self, inputs):
        ...
        if self.use_residual:
            return self.stocastic_depth(self.conv(x)) + inputs
        else:
            return self.conv(x)
